        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.map((item, index) => (
            <div 
              ...
              className='border border-[#C9D8FF] rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 p-4 flex flex-col items-center text-center'
              key={index}
            >
              {/* Doctor Image - circular */}
              <img 
                className='w-24 h-24 object-cover rounded-full bg-[#EAEFFF] mb-3' 
                src={item.image} 
                alt={item.name} 
              />
